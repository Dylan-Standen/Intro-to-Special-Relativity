...
\begin{remark}
    \textit{To see this relation between group elements and algebra elements, notice that a 1-parameter family of group elements $g(t): [t_0,t] \rightarrow G$ with $g(t_0) =g_0$ (initial condition) can be linearised near $t_0$ via}
\begin{equation*}
    g(t_0 + \epsilon) = g(t_0) + \epsilon \underbrace{\dv{g(t)}{t}}_{X}\big|_{t=t_0} + \mathcal{O}(\epsilon^2) \, .
\end{equation*}

\textit{Given the group structure, this implies there exists a $h(\epsilon) = e + \epsilon X + \mathcal{O}(\epsilon^2)$ such that}
...
\textit{This means that for any $\dot{g}(t_0)$ in the tangent space about a point, the corresponding lie algebra element can be found by left multiplying with $g^{-1}(t_0)$. Overall, this implies that}
\begin{equation*}
    g^{-1}(t)\dot{g}(t) = X \, ,
\end{equation*}
