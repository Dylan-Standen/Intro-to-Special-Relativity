...
\]
Simple examples of tensor fields include scalar fields, $T^0_0\mathcal{M}$, and vector fields, $T^1_0 \mathcal{M}$. For our purposes, I shall only concern ourselves with tensor fields of, at most, type $(0,2)$. \\
...
To find the corresponding algebra of a Lie group, it is merely of process of linearising near the identity of the group manifold. The tangent space at the identity is imbued (it follows from the group multiplication law and the Becker-Campbell-Hausdorff formula) with an anti-symmetric, bilinear, binary operation: which is the Lie bracket. For Minkowski space with the isometry group $SO^+(1,3)$, let $\Lambda \in SO^+(1,3)$ and write an infinitesimal Lorentz transformation as
\[
\Lambda^\mu{}_{\nu} = \delta^\mu{}_{\nu} + \omega^\mu{}_{\nu} + \mathcal O(\omega^2) \,.
\]
...
The trivial invariant subspaces are $\emptyset$ and $V$ itself. \\
...
\begin{equation*}
    g(t_0 + \epsilon) = g(t_0) + \epsilon \underbrace{\dv{g(t)}{t}}_{X}\big|_{t=t_0} + \mathcal{O}(t^2) \, .
\end{equation*}
...
\begin{equation*}
e(t) : = \sum_{k=0}^\infty \frac{(tX)^k}{k!} \, .
\end{equation*}
\end{remark}
...
Notice that the rotational generators $J_i$ obey the same algebras as angular momentum operators in QM. This should be unsurprising. The boosts, however, do not form the same algebra. They can be made to do so by extending the field to $\mathbb{C}$, i.e. take linear combinations of vectors with complex coefficients. This leads to the \textit{complexified lie algebra} $\mathfrak{so}(1,3)_{\mathbb{C}}$. Take the combinations
\begin{equation*}
...
Let us now take a brief detour to introduce some `highest weight theory'. But, firstly I must use an important relation, namely the fact that 
...
\begin{proof}
    ...
    \begin{align*}
        \eta\left(\sum_{k =1}^N X_k \otimes z_k\right) \mapsto \sum_{k=1}^N z_kX_k \, \, , \, X \in \mathfrak{su}(2), \, z \in \mathbb{C} \, ,
    \end{align*}
...
\begin{align*}
    ...
    \eta(X \otimes (z+w)) & = (z+w) X \qquad z,w \in \mathbb{C}, \hspace{0.15cm} X \in \mathfrak{su} \, .
\end{align*}
...
I choose the standard basis $\{T_a\}_{a=1}^3$ for $\mathfrak{su}(2)$ with complexification $\{T_a \otimes 1\}_{a = 1}^3 \in\mathfrak{su}(2)_\mathbb{C}$. A general element of $\mathfrak{su}(2)_\mathbb{C}$ can be written as

\begin{equation*}
    X = \sum_{k=1}^3 T_k \otimes z_k \, ,
\end{equation*}
thus
...
\begin{equation*}
    X = \sum_{k=1}^3 T_k \otimes z_k = \sum_{k=1}^3 T_k \otimes 0 = 0 \, .
\end{equation*}
...
For the proper time to be non-complex (which would be odd) for a timelike curve, it is required that $\eta_{\mu \nu}\dot{\Gamma}^\mu(\tau) \dot{\Gamma}^\nu(\tau) = -1$. This then means that the curve $\Gamma$ is parametrised by the proper time. With this, I can now define the 4-velocity of the curve (imagine a particle whose position is dependent on time).
...
Let's look at how this would work. If I vary the action with respect to the curve $\Gamma(s)$. For this, I need to use functional calculus. Just treat it the same as regular calculus, but now the objects of interest are functions of functions. 
...
where $\tau^\prime$ is a root of the function $f(\tau) = t - x^0(\tau)$. Via a similar computation, I find the 3-momentum to be $P^i = mU^i = \gamma m v^i$ (check yourself). If I then take the inner product of the 4-momentum, I find,
...
\begin{remark}
    ...
    \end{equation*}
    \textit{hence $A$ transforms in the manner described above.}
\end{remark}

Now, in a similar fashion the field-strength tensor also transforms under Lorentz transformations. But, not in the same way. Since it is a (2,0) tensor, it takes two covectors as inputs, i.e.,
