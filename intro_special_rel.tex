...
This section of the text will set the stage for quite literally everything to come. We shall quickly review the historical context behind this beautiful mathematical structure before looking (in)formally at its contents. 
...
Before we begin, I should mention that we shall not cover curved spacetimes at all within this text. They are not needed to understand QFT and are only applicable if you were to look at theories defined on curved background geometries. \\
...
In simple terms, a smooth (or differentiable) manifold is one that locally ``looks like" (technically, it must be locally homeomorphic to) $\mathbb{R}^n$ with enough structure to do calculus on. This means that we are able to define continuous functions on it: such as tensor fields. Note, there is, of course, a very thorough topological definition that requires certain conditions, such as `second countable' and `Hausdorff', but we will act like good physicists and ignore the mathematicians' pleas for rigour.  \\

Given a smooth manifold $\mathcal{M}$, we can define a generalisation of a tangent to a curve via the following definition:
...
The tangent space naturally forms a real vector space, allowing us to take linear combinations and scalar multiples of tangent vectors. Tangent vectors can also be viewed as smooth linear operators that act on functions defined on $\mathcal{M}$ that are required to satisfy the Leibniz rule (we will ignore this). If we want to look at the tangent spaces $\forall \,p \in \mathcal{M}$ a natural object is the tangent bundle, 
...
Think of this as merely the set of all tangent vectors to the manifold. An element of \( T\mathcal{M} \) is a pair \( (p, X) \), where \( p \in \mathcal{M} \) and \( X \in T_p\mathcal{M} \) are vectors. As with any vector space, we may find its' dual; namely, the set of linear maps from the vector space to the base field we are working over (usually $\mathbb{R}$ or $\mathbb{C}$):
...
In a similar fashion to our definition of the tangent bundle, we may define: 
...
As we have now introduced both the tangent and cotangent bundles, we may define what is meant by a tensor field and, more generally, the tensor bundle. First, recall that a vector field is simply the assignment of a vector to a specific point in space, hence the tensor field may be defined naturally as 
...
\]
Simple examples of tensor fields include scalar fields, $T^0_0\mathcal{M}$, and vector fields, $T^1_0 \mathcal{M}$. For our purposes, we shall only concern ourselves with tensor fields of, at most, type $(0,2)$. \\
...
With these basic definitions we can now discuss Minkowski space proper. The thing that distinguishes Minkowski space from regular Euclidean 4-space, is the Minkowski metric.
...
A manifold that has the above signature is called \textit{Lorentzian}. We have now ``rigorously'' defined everything we need to completely define Minkowski space:

\fbox{\begin{minipage}{34em}
\begin{definition}
    \textit{Minkowski space}, is the Lorentzian manifold $M = (\mathbb{R}^4, \eta$), where $\eta$ is the Minkowski metric. Since our manifold is flat, we may choose global \textit{inertial coordinates} such that $x \in M$ takes the form $x =(t,x^1,x^2,x^3)$. We also often write $M$ as $\mathbb{R}^{1,3}$.
\end{definition}\end{minipage}}

The differing sign means that the Minkowski metric is not positive definite ($>0$), like the Euclidean metric, but actually indefinite and has 3 distinct possibilities, depending on the sign of the inner product. As per usual, the inner product is defined via the metric tensor of the corresponding space, for example, in Euclidean space the inner product of two vectors, $v,w \in \mathbb{R}^n$ can be written as $\delta_{ij}v^i w^j$. We may also do the same in Minkowski space, where we now take our indices to be Greek (I assume you know Einstein's summation convention).  \\
...
If we define the inverse metric, written $\eta^{\mu \nu}$ via, 
...
As previously mentioned, we may choose a global orthonormal basis such that $\eta$ takes the regular form. We are, of course, well within our right to choose a different basis for our space. This observation leads to the Lorentz group - which we shall now cover.
...
As a motivational example we shall look initially at $SO(n)$, before looking at the Lorentz group. Recall that $SO(n)$ is a subgroup of the \textit{isometry group} of $\mathbb{R}^n$, this means that the quadratic form is preserved. Namely, given the quadratic form
...
which implies that $A^\text{T}gA =g$. This is exactly the requirement for the matrix Lie group $O(n)$ (when $g = I)$. Think of these transformations as rotations; the vector's norm hasn't changed, it has simply been rotated. If we wish to preserve the orientation of the vector, i.e. no flips, we require only proper rotations, our group then becomes $SO(n)$ - the group of matrices in $O(n)$ with $\text{det} = 1$. \\
...
There is also another nuance; time has a different sign to all other components in our space. In order to impose time orientation (since $\text{det} = 1$ imposes spatial orientation) of our space we also require that the time component of our transformation, $\Lambda^0_0 \geq 1$. With this time orientation, our group is then $SO^+(1,3)$ the \textit{Proper Orthochronus Lorentz Group}.\\

The next logical place for us to cover is the Lie algebra of the Lorentz group, I shall assume a certain amount of knowledge of Lie algebras, but, don't worry it won't be too in depth  - I won't be covering anything like Cartan Matrices, Cartan-Weyl bases, or anything other than the most basic representation theory.
...
To begin I will firstly introduce the, hopefully familiar, definition of a Lie algebra: 

\fbox{\begin{minipage}{34em}
\begin{definition}
    Let $G$ be a matrix Lie group, its corresponding Lie algebra, $\mathfrak{g}$, is a vector space along with the product $[\cdot, \cdot]:\mathfrak{g}\times \mathfrak{g} \rightarrow\mathbb{F}$. It admits a basis of (in general) matrices, often called the \textit{generators} of the Lie algebra.
\end{definition}\end{minipage}}

To find the corresponding algebra of a Lie group we linearise near the identity of our group manifold. The tangent space at the identity is imbued (it follows from the group multiplication law and the Becker-Campbell-Hausdorff formula) with an anti-symmetric, bilinear, binary operation: the Lie bracket. For Minkowski space with the isometry group $SO^+(1,3)$ we let $\Lambda \in SO^+(1,3)$ and write an infinitesimal Lorentz transformation as
\[
\Lambda^\mu{}_{\nu} = \delta^\mu{}_{\nu} + \omega^\mu{}_{\nu} + \mathcal O(\omega^2) \,.
\]
...
\fbox{\begin{minipage}{34em}
\begin{definition}
    Given a Lie group $G$, a representation of $G$ is a linear map $D: G \rightarrow GL(n, V)$. Where $V$ is the vector space over which the matrices act, i.e.
    \begin{equation*}
        v \mapsto D(g)v \, \in V\, ,
    \end{equation*}
...
Think of a representation as simply taking an element of our group and ``representing'' it with a matrix from the general linear group. There is also an analogous definition for Lie algebras. I will not cover this. In physics, we often deal with representations that cannot be split into sub-representations. It will hence be important to quickly define what we mean by this:

\fbox{\begin{minipage}{34em}
\begin{definition}
    A representation $D: G \rightarrow GL(n, V)$ is said to be \textit{irreducible} if it does not possess any non-trivial invariant subspaces.   
\end{definition}\end{minipage}}

The obvious trivial invariant subspaces are $\emptyset$ and $V$ itself. \\

We now form a basis of generators for our Lie algebra by defining the generators $M^{\mu\nu}=-M^{\nu\mu}$ such that 
\[
...
\]
This relation between the group elements and Lie algebra elements is called the \textit{exponential map}. These generators satisfy the Lorentz algebra commutator relations:
\[
\
[M^{\mu\nu},\,M^{\rho\sigma}] =
...
We often split this basis into rotations and boosts via the following relations
Let
\[
...
\]
Notice that the rotational generators $J_i$ obey the same algebras as angular momentum operators in QM. This should be unsurprising. The boosts, however, do not form the same algebra. We can make them do so by extending our field to $\mathbb{C}$, i.e. we now take linear combinations of vectors with complex coefficients. This leads to the \textit{Complexified Lie algebra} $\mathfrak{so}(1,3)_{\mathbb{C}}$. We take the combinations
\[
...
\begin{proof}
    To show there exists an isomorphism we must show that $\exists$ a bijection between the algebras. Let's firstly see exactly what we mean by $\mathfrak{su}(2)_\mathbb{C}$:

    \begin{equation*}
        \mathfrak{su}(2)_\mathbb{C} = \mathfrak{su}(2) \otimes_\mathbb{R}\mathbb{C} 
    \end{equation*}
    Now, we should construct a  bijection (injective and surjective) between the two sets. I shall define the map $\eta : \mathfrak{su}(2)_\mathbb{C} \rightarrow \mathfrak{sl}(2,\mathbb{C})$ by

    \begin{align*}
        \eta(X \otimes z) \mapsto zX \, \, , \, X \in \mathfrak{su}(2), \, z \in \mathbb{C} \, .
    \end{align*}
This map is $\mathbb{R}$-linear in the first argument (since $\mathfrak{su}(2)$ is a real vector space) and $\mathbb{C}$-linear in the second, i.e. 

\begin{align*}
    \eta((aX + bY) \otimes z) & = z(aX + bY)\\
    \eta(X \otimes (z+w)) & = (z+w) X \, .
\end{align*}

For it to be a valid Lie algebra isomorphism, it must preserve the Lie bracket: let's check

\begin{align*}
    \eta\left([X \otimes z, Y \otimes w]  \right) & = \eta \left( [X,Y] \otimes zw\right)\\
    & = zw[X,Y]\\
    & = [zX, wY]\\
    & = [\eta(X \otimes z), \eta(Y \otimes w)]
\end{align*}

It can be then shown (I exclude for brevity) that this map is also injective and surjective. 
\end{proof}
...
Given these facts, we are then free to write the two distinct sets of spin-quantum numbers of the two algebras in $\mathfrak{so}(1,3)_\mathbb{C} = \mathfrak{sl}(2, \mathbb{C}) \oplus \mathfrak{sl}(2, \mathbb{C})$ as $(j_L, j_R)$. Where in physics we define $j_L/ j_R = \frac{\ell_{L/R}}{2}$. These numbers encode differing representations of the Lorentz group. 


The Lorentz group is actually what's known as \text{non-compact} (imagine infinite volume), thus it can be shown (take my word for it) that there exists no non-trivial finite-dimensional unitary representations. This is bad for any quantum theory since we require our theories to be unitary. This means that particles can't be described with this alone; we actually need the \textit{Poincar\'e} group (more precisely a unitary representation of it). \\

The Poincar\'e group is the Lorentz group $+$ translations (technically it is a semi-direct product group - but we won't worry about that here), with single particle states being described by unitary irreducible reps of the Poincar\'e group. They are indexed by both spin and mass. This topic is slightly beyond the main aim of this text; it is, however, very interesting, so I suggest reading further about it. 

\subsubsection{Representations of the Lorentz Group:}

We shall now quickly look at certain representations of the Lorentz group i.e. different values of $(j_L,j_R)$:
...
Now that we have covered the group theory of the Lorentz group, let's now actually look at some relativistic dynamics. Note, being as terrible with constants as I am, I shall use natural units: we set $c = 1$. With this choice of units, Einstein's famous relation is now just $E = m$. This is often the case in most theory texts as it makes things a lot cleaner. It can, however, lead to issues if you forget to put them back, when all said and done. 


\subsection{4-Vectors:}

Given our Minkowski space $\mathbb{R}^{1,3}$ a point on the manifold is called an event or 4-position and (as stated earlier) takes the form $x^\mu = (t,x^1,x^2,x^3)$. As you know, time in relativity is relative. Hence, it would not make sense to define the usual things like velocity or acceleration using the regular coordinate time. This would mean that the velocity of an object would be frame dependent i.e. a race-car could be one velocity for the driver and another velocity for an observer. We instead use something called the proper time, which I shall now introduce:
...
For the proper time to be non-complex (which would be odd) for a timelike curve, we require the condition $\eta_{\mu \nu}\dot{\Gamma}^\mu(\tau) \dot{\Gamma}^\nu(\tau) = -1$. This then means that the curve $\Gamma$ is parametrised by the proper time. With this, we can define the 4-velocity of the curve (Imagine a particle whose position is dependent on time).
...
\begin{definition}
    ...
    \begin{equation*}
        U^\mu = \pdv{\Gamma^\mu(\tau)}{\tau} = \pdv{(t(\tau), x^1(\tau), x^1(\tau), x^2(\tau))}{\tau} \, .
    \end{equation*}

    Where we have used local coordinates to describe the curve $\Gamma$. 
\end{definition}\end{minipage}}
...
\end{align*}
Just as we have defined the 4-velocity we are free to define the 4-acceleration:
...
Given the normalisation condition on the 4-velocity we find, 
\begin{align*}
   \pdv{}{\tau} \left(\eta_{\mu \nu} U^\mu U^\nu \right) & = 0, \\
   \implies \eta_{\mu \nu}U^\mu A^\nu & = 0\, .
\end{align*}
...
I hope you're familiar with the action functional and the concept that the equations of motion come from extremising  said action. Essentially, in this case, we are looking for a curve such that $\delta \tau= 0$. \\


Let's look at how this would work if we vary the action with respect to the curve $\Gamma(s)$. For this, we need functional calculus. Just treat it the same as regular calculus, but now we are operating on functions of functions. 
...
Similar to finding the extrema in regular calculus we demand that the variation, $\delta\tau = 0$. Thus, using the Euler-Lagrange equations, we find

\begin{align*}
    \pdv{}{\sigma}\left(\frac{1}{\sqrt{-\eta_{\mu \nu}\dot{\Gamma}^\mu(\sigma)\dot{\Gamma}^\nu(\sigma)}} \cdot \dot{\Gamma}_\beta(\sigma) \right) & = 0 \\
    \implies \ddot{\Gamma}_\beta(\sigma) & = 0 \, .
\end{align*}
where the derivative of the square root vanished due to the normalisation condition $\eta_{\mu \nu}\dot{\Gamma}^\mu(\tau) \dot{\Gamma}^\nu(\tau) = -1$. Since, for a timelike curve, we are free to set $\sigma = \tau$, we have (raising the index) 
...
Let's look at the case where we have a relativistic point particle of mass $m$ parametrized by $\tau$, and localized at a point $y$. In this case we have the energy-momentum tensor,

\begin{equation*}
    T^{\mu\nu} = m \int \text{d}\tau \, \, U^\mu(\tau)U^\nu(\tau) \delta^4(x - y(\tau))\, .
\end{equation*}

It is a distribution with support along the world-line parametrized by $\tau$. Using the energy-momentum tensor, we are able to form the 4-momentum,
...
\[
E= P^0=\int T^{00}\,\text{d}^3 x,\qquad
P^i=\int T^{0i}\,\text{d}^3x 
 \, .
\]
\end{definition}\end{minipage}}

This then allows us to compute the 4-momentum for the above energy-momentum tensor, via
\begin{align*}
    E & = \int_{\mathbb{R
    }^3} \text{d}^3x \, \, T^{00} \\
...
where $\tau^\prime$ is a root of the function $f(\tau) = t - x^0(\tau)$. Via a similar computation, we find the 3-momentum to be $P^i = mU^i = \gamma m v^i$ (check yourself). If we then take the inner product of the 4-momentum, we find,
...
Thus, we see that the inner product is 
...
\fbox{\begin{minipage}{34em}
\begin{definition}
Suppose we have $n$ particles each with an associated 3-momentum $\boldsymbol{p}_n$, then the \textit{centre of momentum frame} is the inertial frame such that,
...
In such a frame, the norm of the total 3-momentum is clearly zero. Hence, for a collection of particles, the centre of momentum frame is also the rest frame. This frame is useful in multi-particle collisions since we may set the sum of all momenta to be zero thereby simplifying our analysis. While it is good to know that it exists, it does not come up (I think at all) in the courses you wish to read.
...
To discuss classical Electrodynamics, we must first discuss Maxwell's theory of electromagnetism. As well as its gauge symmetries. 
...
\end{equation*}
the 4-derivative. Does this transformation really leave the physics unchanged? Well, yes. I will quickly show this.
...
\begin{proof}
    ...
    \end{equation*}
    we can see that the $\boldsymbol{E}$ field transforms as 
    \begin{align*}
...
This is good! The things we can actually measure using EMF devices are completely gauge invariant. This redundancy allows us to the \textit{choose a gauge}. Certain gauges will make calculations much simpler. Common gauge include:
...
\begin{itemize}
    \item The \textit{Lorenz Gauge}, 
           \begin{equation*}
               \partial_\mu A^\mu = 0 .
           \end{equation*}
           We are always free to choose this gauge, to see this notice that if we are given a gauge field $A^{\prime \mu}$ such that $\partial_\mu A^{\prime \mu} = f(t, \boldsymbol{x})$, we can then choose $\omega$ such that
           \begin{equation*}
               \partial_\mu \partial^\mu \omega = -f \, .
           \end{equation*}
           Our gauge potential is then
           \begin{equation*}
    ...
    \end{equation*}
    We are free to choose this gauge using the same argument as for the Lorenz gauge. It turns out that the $A_0$ component is purely specified by $\nabla \cdot \boldsymbol{A}$, we may set 
    \begin{equation*}
        A_0 = 0\, .
    \end{equation*}
...
\fbox{\begin{minipage}{34em}
\begin{definition}
The \textit{Field-Strength tensor} is an anti-symmetric (2,0) tensor, written in terms of the gauge potential as 
...
It should be clear to see that this tensor is completely gauge invariant (please check yourself). In terms of $\boldsymbol{E}$ and $\boldsymbol{B}$, the (contravariant) Field-Strength tensor can be written as 
...
For the remaining Maxwell equations we use something called the \textit{Bianchi Identity} to write them as 
...
As we alluded to earlier, the theory of Electromagnetism is a relativistic theory. This becomes obvious once we introduce the gauge potential and field-strength tensor. This is because they transform naturally under Lorentz transformations. For example, the gauge potential transforms as 

\begin{equation*}
    A^\mu \rightarrow A^{\prime \mu} = \Lambda^\mu_\nu A^\nu \, .
\end{equation*}

In the language of representation theory, we would say it transforms under the vector (sometimes called \textit{defining}) rep of $SO^+(1,3)$. 
...
\begin{remark}
    ...
    \begin{equation*}
        A = A_\mu dx^\mu \, ,
    \end{equation*}
    \textit{where $dx^\mu$ is a dual basis (of 1-forms). In this way we have} 
    \begin{equation*}
        dx^\mu \rightarrow \Lambda^\mu_\nu dx^\nu \, ,
    \end{equation*}
    \textit{hence $A$ transforms in the way seen above.}
\end{remark}

Now, in a similar fashion the field-strenth tensor also transforms under Lorentz transformations. But, not in the same way. Since it is a (2,0) tensor, we see it takes two covectors as inputs, 

\begin{equation*}
    F^{\mu \nu} = F(dx^\mu, dx^\nu)  \, ,
\end{equation*}

under a Lorentz transformation we then have 

\begin{equation*}
    F^{\mu \nu} \rightarrow F^{\prime \mu \nu} = \Lambda^\mu_\rho F^{\rho \eta}\Lambda_\eta^\nu \,.  
\end{equation*}

In vector notation we have $F^\prime = \Lambda F \Lambda^{-1} \, .$ It is said to transform in the \textit{adjoint} rep of $SO^+(1,3)$. Given that they transform under Lorentz transformations, we can form Lorentz invariant quantities. Which is great when we want to consider field theories. Since, the Lagrangian densities (and hence actions) we build should be completely Lorentz invariant. This fact also constrains the form of the action to Lorentz invariants built from the field-strength. \\
...
This quantity is a Lorentz scalar, which is exactly what we need. This is the Lagrangian density used in QED (up to constants).
